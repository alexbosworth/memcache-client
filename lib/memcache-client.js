...
// -------
//
// A cluster represents a collection of memcache clients.
...
// ------
//
// The client represents a connection to one memcache server.

function Client(host, port, adapter) {
...
Client.prototype.flushQueue = function (cb) {
...
Client.prototype.connect = function (cb) {
  ...
  this.sock.on('error', function() {
    self.sock = undefined;
    self.flushQueue("CONNECTION_ERRROR", 'No connection to server.');
    if (self.retry) { self.reconnect(); }
  });
};
...
Client.prototype.rollback = function(message) {
...
Client.prototype.sendServer = function (command, value) {
  ...
  if (!this.sock) {
    this.rollback('No Connection Available.');
    return;
  }
  try {
