...
function Client(servers, adapter, strategy) {
  ...
  this.strategy = strategy || Strategy.hash;
...
Connection.prototype.sendServer = function (handler, command, value) {
  // Lost our connection. Expect the error callback to be handled by error event, not here.
  if (!this.sock) {
    handler.callback({ type: 'CONNECTION_ERROR', description: 'No Connection Available.'}, null);
    return;
  }
  try {
    ...
    handler.callback({ type: 'CONNECTION_ERROR', description: 'Lost connection to server.' }, null);
  }
};
