...
MemcacheClient.prototype.prepConnection = function(handler, cb) {
  if (!this.sock) {
    cb({ type: 'CONNECTION_ERROR', description: 'No Connection to Server'}, null);
    return false;
  } else {
    this.queue.enqueue({
      handler: handler,
...
MemcacheClient.prototype.store = function(operation, key, val, opts, cb) {
  ...
  val = String(val);
  if (key.length > 250) {
    cb({ type: 'CLIENT_ERROR', description: 'Key too long, max 250 char'}, null);
    return null;
  }
...
MemcacheClient.prototype.stats = function (type, cb) {
  if (!cb) { 
    cb = type;
    type = '';
  }
  ...
  case 'slabs':
    ...
    break;
  case '': // Forward compatible: treat any future state type as matching the default pattern.
  default:
...
MemcacheClient.prototype.connect = function (cb) {
  var self = this;
  ...
  this.sock = net.connect(this.port, this.localhost, function () {
    ...
    self.sock.addListener('error', function () {
	...
	while (lost) {
	  lost.callback({ type: "CONNECTION_ERROR", description: 'Lost Connection to Server'}, null);
...
MemcacheClient.prototype.sendServer = function (command, value) {
  if (!this.sock) {
    this.queue.pop().callback({ type: 'CONNECTION_ERROR', description: 'No Available Connection'}, null);
  }
  try {
    ...
    this.queue.pop().callback({ type: 'CONNECTION_ERROR', description: x.message }, null );
  }
};
...
MemcacheClient.prototype.processBuffer = function () {
  while (this.buffer.length > 0) {
    ...
    if (!peekResponse.str) { // No full line available. Need more data off the wire.
      ...
      this.queue.dequeue().callback({type: peekResponse.str, description: ''}, null);
    } else {
